<script setup>
import { useBrands, useNotification } from '@/stores';
...
const getSingleBrandData = async() =>{
    let res = await brand.getSingleBrandData(route.params.id);
    if(res.result){
        brandName.value = res.result?.name;
...
const submit = async() =>{
  ...
  const res = await brand.updateBrand(route.params.id, formData);
  if(res.result){
      ...
      <div class="card">
        <div class="card-header">
          <div class="card-tools">
            <div class="mb-2 d-flex justify-content-end">
              <router-link :to="{ name: 'brand-list' }" class="btn btn-info ml-2"><i class="fas fa-arrow-left mr-2"></i>Back</router-link>
            </div>
          </div>
        </div>
        <!-- ./card-header -->
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input type="text" id="inputName" class="form-control" placeholder="Enter Product English Name" v-model="brandName"/>
                      <span v-if="errors">
          ...
          <div class="d-flex justify-content-end">
            <button type="button" class="btn btn-info" v-if="brand.loading"><i class="fas fa-spinner fa-spin"></i> Loading...</button>

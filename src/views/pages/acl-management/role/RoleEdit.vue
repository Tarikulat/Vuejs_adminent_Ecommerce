...
const getRoleDetailsInfo = async() =>{
  const res = await aclManagement.getRoleDetailsInfo(route.params.id);
  if(res.result){
          name.value          = res.result?.name;
          displayName.value   = res.result?.display_name;
          description.value   = res.result?.description;
    const rolePermission      = res.result?.permissions.map(s => s.id);
          permissionIds.value = rolePermission;
  }
}
...
const submit = async() =>{
  const res = await aclManagement.updateRole(route.params.id, {
  ...
  if(res.result){
    ...
    notification.Success(`${displayName.value} Updated Successfully`);
  }else{
      ...
      <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <input type="text" id="displayName" class="form-control" placeholder="Enter Display Name" v-model="name"/>
                      <span v-if="errors">
          ...
          <div class="d-flex justify-content-end">
            <button type="button" class="btn btn-info" v-if="aclManagement.loading"><i class="fas fa-spinner fa-spin"></i> Loading...</button>
            <button type="button" class="btn btn-info" @click.prevent="submit" v-else>Update Role</button>
          </div>
        </div>
      </div>
      <!-- /.card -->

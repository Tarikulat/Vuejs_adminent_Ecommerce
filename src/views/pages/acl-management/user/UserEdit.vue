...
const getUserDetailsInfo = async() =>{
  const res = await user.getUserDetailInfo(route.params.id);
  if(res.result){
    userName.value = res.result?.name;
    phone.value = res.result?.phone_number;
    email.value = res.result?.email;
    roleId.value = res.result?.roles[0]?.id;
    status.value = res.result?.is_active
  }
}
...
const submit = async() =>{
  ...
  const res = await user.updateUser(route.params.id, formData);
  if(res.result){
      ...
      <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <input type="text" id="password" class="form-control" placeholder="Reset Your Password" v-model="password"/>
                      <span v-if="errors">
          ...
          <div class="d-flex justify-content-end">
            <button type="button" class="btn btn-info" v-if="user.loading"><i class="fas fa-spinner fa-spin"></i> Loading...</button>

...
const editProductInfo = async() =>{
  const res = await product.editProductInfo(route.params.id);
  if(res.result){
          name.value                  = res.result?.name;
          categoryId.value            = res.result?.category_id;
          subCategoryId.value         = res.result?.sub_category_id;
          brandId.value               = res.result?.brand_id;
          status.value                = res.result?.status;
          productType.value           = res.result?.type;
          isFreeShipping.value        = res.result?.free_shipping;
          stock.value                 = res.result?.current_stock;
          stock.value                 = res.result?.current_stock;
          shortDescription.value      = res.result?.short_description;
          description.value           = res.result?.description;
          dbImages.value              = res.result?.images;
          thumbnail.value             = res.result?.image;
          metaTitle.value             = res.result?.meta_title;
          metaKeywords.value          = res.result?.meta_keywords;
          metaDescription.value       = res.result?.meta_description;
          buyPrice.value              = res.result?.buy_price;
          regularPrice.value          = res.result?.mrp;
          offerPrice.value            = res.result?.offer_price;
          dynamicInputs.value         = res.result?.product_prices;
          videoUrl.value              = res.result?.video_url;
    const weightIds                   = res.result?.product_prices.map(s=>s.pivot.size_id);
          weightId.value              = weightIds
    const BuyPrice                    = res.result?.product_prices.map(s=>s.pivot.buy_price);
          variationBuyPrice.value     = BuyPrice
    const Default                     = res.result?.product_prices.map(s=>s.pivot.is_default);
          isDefault.value             = Default
    const MrpPrice                    = res.result?.product_prices.map(s=>s.pivot.mrp);
          variationRegularPrice.value = MrpPrice
    const OfferPrice                  = res.result?.product_prices.map(s=>s.pivot.offer_price);
          variationOfferPrice.value   = OfferPrice;
  }
}

name.value = product.productDetails?.name;
...
const deleteDbImg = (index, id) => {
  galleryImageIds.value.push(id);
...
const submit = async() => {
  const formData = new FormData();
    ...
    if(imagePreview.value !=0){
      for (const image of images.value) {
        formData.append('gallery_images[]', image);
      }
    }

    if(galleryImageIds.value.length > 0){
      for (const ids of galleryImageIds.value){
        formData.append('gallery_image_ids[]', ids);
      }
    }else{
      formData.append('gallery_image_ids[]', galleryImageIds.value);
    }
  ...
  const res = await product.updateProduct(route.params.id, formData);

  if(res.result){
    router.push({name: 'product-list'})
    notification.Success(`${name.value} Updated Successfully`);
    product.getProducts();
  }else{
      ...
      <div class="card">
          ...
          <div class="card-body">
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="card card-info">
                      <div class="card-header d-flex edit-info-header">
                      ...
                      <div class="card-body">
                        <div class="form-group">
                          <label for="inputName">Product Name(English)</label>
                          ...
                          <label for="inputDescription">Thumbnail</label>
                            <div class="row">
                              <div class="col-md-12">
                                  ...
                                  <span class="img-remove-btn" style="position:relative" v-if="thumbnailPreview || thumbnail">
                                    <img width="100" class="gallery_img" :src="thumbnailPreview? thumbnailPreview : thumbnail" alt="">
                                  </span>
                              </div>
                            </div>
                        </div>
                          ...
                          <label for="inputDescription">Gallery Images</label>
                            <div class="row">
                              <div class="col-md-12">
                                  ...
                                  <span class="img-remove-btn" style="position:relative" v-for="(image, index) in dbImages" :key="index">
                                    <img width="100" class="gallery_img" :src="image.image" alt="">
                                    <span class="close-btn bg-danger text-light" @click="deleteDbImg(index, image.id)"><i class="fas fa-times"></i></span>
                                  </span>
                              </div>
                            </div>
                        </div>
              ...
              <div class="modal-footer justify-content-end">
                <button type="button" class="btn btn-info" v-if="product.loading"><i class="fas fa-spinner fa-spin"></i> Loading...</button>

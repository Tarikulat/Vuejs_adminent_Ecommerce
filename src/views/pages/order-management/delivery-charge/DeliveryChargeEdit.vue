...
const deliveryChargeSingleData = async() =>{
  let res = await deliveryCharge.deliveryChargeSingleData(route.params.id);
  if(res.result){
    deliveryAreaName.value    = res.result?.name;
    deliveryAreaCharge.value  = res.result?.delivery_fee;
    deliveryAreaMinTime.value = res.result?.min_time;
    deliveryAreaMaxTime.value = res.result?.max_time;
    deliveryTimeUnite.value   = res.result?.time_unit;
    deliveryStatus.value      = res.result?.status;
  }
} 

const submit = async() =>{
    const res = await deliveryCharge.updateGateway(route.params.id, {
        name        : deliveryAreaName.value,
        delivery_fee: deliveryAreaCharge.value,
        min_time    : deliveryAreaMinTime.value,
        max_time    : deliveryAreaMaxTime.value,
        time_unit   : deliveryTimeUnite.value,
        status      : deliveryStatus.value,
    });
    if(res.result){
        router.push({name:'charge-list'})
        notification.Success(`Delivery Gateway Updated Successfully`);
    }else{
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="d-flex justify-content-end">
            <button type="button" class="btn btn-info" v-if="deliveryCharge.loading"><i class="fas fa-spinner fa-spin"></i> Loading...</button>

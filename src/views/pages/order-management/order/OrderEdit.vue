...
const changeableChargeValue = ref('');
...
const getSingleOrder = async() =>{
    let res = await order.getSingleOrder(route.params.id);
    if(res.status === 200){
              customerName.value          = res.data?.result?.customer_name;
              customerPhone.value         = res.data?.result?.phone_number;
              customerDistrict.value      = res.data?.result?.district;
              customerAddress.value       = res.data?.result?.address_details;
              orderStatusId.value         = res.data?.result?.status_id;
              deliveryChargeId.value      = res.data?.result?.dg_id;
              paymentStatus.value         = res.data?.result?.is_paid? 1 : 0;
              spacialDiscount.value       = res.data?.result?.special_discount;
              changeableChargeValue.value = res.data?.result?.delivery_charge;
              advancePayment.value        = res.data?.result?.advance_payment;
              preparedBy.value            = res.data?.result?.prepared_by?.name;
        ...
        for(let item of items){
            let orderItem = {
                ...
                offer_price   : item.pivot?.item_sell_price,
                mrp           : item.pivot?.item_mrp,
                discount      : item.pivot?.item_discount,
                weight_id     : item.pivot?.size_id,
                quantity      : item.pivot?.quantity,
                product_prices: item.product_prices,
                pivot         : item.pivot,
            }
            let res = await cart.addToCart(orderItem);
            if(res){
                cartItems.value = res;
            }
        }
...
const removePreparedBy = async() =>{
    preparedBy.value = null;
}
...
const submit = async() =>{
    let res = await order.updateOrder(route.params.id, cartItems.value, {
                ...
                <div class="col-md-3">
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                <label for="delivery Area">Prepared By</label>
                                <div v-if="preparedBy">
                                    <span class="prepared-by">{{ preparedBy }} <i class="fas fa-times text-danger ml-2" style="cursor:pointer;" @click.prevent="removePreparedBy"></i></span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card card-info">
                        ...
                        <div class="card-footer">
                            <div class="d-flex justify-content-end">
                                ...
                                <button type="submit" class="btn btn-info" @click="submit" v-else> Update Order</button>
                            </div>
                        </div>
                    </div>
                </div>

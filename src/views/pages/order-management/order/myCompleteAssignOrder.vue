...
const myOrderAssignList = async(page=1) =>{
    let res = await order.myOrderAssignList(page, paginateSize.value, searchQuery.value);
    if(res.success){
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title d-flex" style="float:right;">
                        ...
                        <router-link :to="{name:'order-assign'}" class="btn btn-info mb-2 ml-2" style="width:150px;"><i class="fas fa-arrow-left"></i> Back</router-link>
                    </div>
                    <ul class="pagination pagination-sm mb-0" v-if="orders">
                        <Bootstrap5Pagination
                            :data="orders"
                            @pagination-change-page="myOrderAssignList"
                            :limit="3"
                        >
                            <template #prev-nav>

...
const getSingleContactInfo = async() =>{
    const res = await socialContact.getSingleContactInfo(route.params.id);
    if(res.success){
...
const contactType = () =>{
    ...
    if(type.value == 'Twitter'){
        contactText.value = 'Twitter Profile Link'
    }
    if(type.value == 'Youtube'){
        contactText.value = 'Youtube Profile Link'
    }
    if(type.value == 'Linkedin'){
        contactText.value = 'Linkedin Profile Link'
    }
    if(type.value == 'Email'){
...
const submit = async() =>{
    const res = await socialContact.update(route.params.id, {
        title  : title.value,
        status : status.value,
    ...
    if(res.success){
        ...
        notifications.Success("Social Contact Updated Successfully");
    }else{
